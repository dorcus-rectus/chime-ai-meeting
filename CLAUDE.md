...
Amazon Chime SDK + **Amazon Bedrock AgentCore** を使った AI ビデオ会議システム。ユーザー認証 (Cognito)、ユーザー管理 (登録・削除)、RAG (S3 Vectors + Titan Embeddings)、RAG 管理 (一覧・削除)、PDF/テキストファイル登録、画面共有フレーム解析 (AgentCore Vision)、S3 署名付き URL + SQS 非同期インジェスト、利用記録 (DynamoDB) を備える。
...
- S3 一時アップロードバケット: `chime-ai-raw-uploads-${account}` — CORS 設定済み、ライフサイクル 1日で自動削除
- RawUploadS3Policy 権限: `s3:PutObject` (署名付き URL 発行用), `s3:GetObject`, `s3:DeleteObject`
- SQS キュー (`chime-ai-ingest-document`): 可視性タイムアウト 1800秒、DLQ 付き
- Lambda `get-upload-url`: S3 署名付き PUT URL 発行 (GET /documents/upload-url → 5分有効)
- Lambda `ingest-document`: s3Key 受け取り・所有者検証・SQS 送信 → 202
- Lambda `ingest-document-worker`: S3 から content 読み込み → 埋め込み + S3 Vectors 書き込み → S3 削除 (SQS トリガー)
...
- `GET /documents/upload-url` → `chime-ai-get-upload-url` Lambda (S3 署名付き PUT URL 発行)
- `POST /documents` → `chime-ai-ingest-document` Lambda (s3Key 受け取り・SQS 送信 → 202)
...
| `RAW_UPLOAD_BUCKET` | S3 一時アップロードバケット名 |
...
### `get-upload-url/index.ts` — S3 署名付き PUT URL 発行

1. `GET /documents/upload-url` を受け取る
2. `${userId}/${uuid}` 形式の s3Key を生成
3. `@aws-sdk/s3-request-presigner` で 5分有効の S3 署名付き PUT URL を発行
4. `{ uploadUrl, s3Key }` を返す

### `ingest-document/index.ts` — SQS への受付 (202 返却)

1. `POST /documents { s3Key, source }` を受け取る
2. s3Key が `${userId}/` プレフィックスで始まることを検証 (他ユーザーのファイル横取り防止)
3. SQS キューに `{ s3Key, source, userId, tags, isPublic }` を送信 → 202 即時返却
   ※ コンテンツ本体は S3 に格納済みのためメッセージは数百バイト (256KB 制限を回避)

### `ingest-document-worker/index.ts` — RAG インデックス構築

1. SQS イベントから `s3Key` を取得
2. `GetObjectCommand` で S3 からコンテンツを読み込み (サイズ上限: S3 の 5GB)
3. スライディングウィンドウでチャンク分割 (500字、50字オーバーラップ)
4. 並列 5 件で Titan Embeddings V2 ベクトル化
5. S3 Vectors に 25件ずつバッチ書き込み
6. `DeleteObjectCommand` で S3 一時ファイルを削除
7. 失敗メッセージのみ `batchItemFailures` に返す (部分失敗対応)
...
### DocumentUpload — PDF / テキストファイル対応・S3 署名付き URL アップロード
...
- **3 ステップアップロードフロー** (SQS の 250KB 制限を回避):
  1. `GET /documents/upload-url` → `{ uploadUrl, s3Key }` を取得
  2. `PUT uploadUrl` でテキストを S3 に直接アップロード (API Gateway を通さず 5GB まで対応)
  3. `POST /documents { s3Key, source, tags, isPublic }` でインデックス登録をキュー投入
