...
Amazon Chime SDK + **Amazon Bedrock AgentCore** を使った AI ビデオ会議システム。ユーザー認証 (Cognito)、ユーザー管理 (登録・削除)、RAG (S3 Vectors + Titan Embeddings)、画面共有フレーム解析 (AgentCore Vision)、SQS 非同期インジェスト、利用記録 (DynamoDB) を備える。
...
2. 画面共有フレーム (`frame`) があれば RAG をスキップ (Vision 優先)
3. ユーザー発話を Titan Embeddings V2 でベクトル化 → S3 Vectors で上位3件検索
4. RAG コンテキストを XML タグ (`<context>`, `<user_input>`) で組み立て
5. **AgentCore InvokeAgent** を呼び出し (sessionId = Chime MeetingId)
   - 画面共有フレームは `sessionState.files` で添付 (マルチモーダル Vision)
   - レスポンスは AsyncIterable — `for await` でチャンクを結合
6. Polly Kazuha (Neural) で音声合成 → Base64 MP3
7. 利用記録を DynamoDB に非同期保存 (`ragUsed`, `frameAnalyzed` フラグ付き)
...
- `useAIConversation`: AI 会話送受信 + Polly 音声再生 (AudioContext)
