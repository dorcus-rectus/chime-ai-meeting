...
| **音声認識 UI** | マイクボタン 4 状態 (ミュート赤/聴取中シアン/AI処理中琥珀/AI発話中) + ステータスラベル |
| **ぼかし設定** | UserProfile で preference 保存 → 次の会議開始時に自動適用 |
| **RAG 分離** | S3 Vectors クエリを userId でフィルタしてユーザー間のドキュメント漏洩を防止 |
| **テスト** | Vitest 31 テスト + Playwright E2E (helpers + 4 spec) + CDK Jest 53 テスト |
...
cd frontend
npm test              # ウォッチモード
npm run test -- --run # CI モード (ウォッチなし、31テスト)

# ── CDK インフラテスト (Jest + スナップショット + cdk-nag) ──
cd cdk
npm test              # 全テスト実行 (53件)
npm run test:update   # スナップショット更新 (Lambda コード変更後など)
...
```bash
cd frontend && npm run lint  # ESLint (react-hooks/exhaustive-deps: error)
```
