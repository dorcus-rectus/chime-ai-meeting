...
| **RAG** | S3 Vectors + Titan Embeddings V2 + S3 署名付き URL + SQS 非同期インジェスト (上限 5GB) |
...
ブラウザ (React + Amplify Hosting)
  ...
  │     ├── GET  /documents/upload-url → Lambda: get-upload-url → S3 署名付き PUT URL 発行
  │     ├── POST /documents       → Lambda: ingest-document → SQS → ingest-document-worker → S3 Vectors
  │     │     └── (フロントエンドが S3 に直接 PUT → s3Key を通知する 3 ステップフロー)
...
├── rag-filetypes.spec.ts       # txt/md/csv 登録・各ファイル形式のテキスト抽出
...
│   │   ├── ingest-document/index.ts        # RAG 受付（s3Key 受け取り・SQS 送信 → 202）
│   │   ├── ingest-document-worker/index.ts # RAG インデックス構築（S3 読み込み → SQS 非同期）
│   │   ├── manage-documents/index.ts       # RAG 一覧・削除
│   │   └── user-management/index.ts        # ユーザー情報取得・削除
│   └── test/                               # CDK Jest テスト（58 件）
├── frontend/
│   ├── src/
│   │   ├── App.tsx                         # 画面遷移
│   │   ├── hooks/
...
ベクトルキーを `${userId}/uuid` 形式で管理し、クエリ時に userId でフィルタすることでユーザー間のドキュメント漏洩を防止。

ドキュメント登録は **3 ステップの S3 署名付き URL フロー**で SQS の 256KB 制限を回避し、最大 5GB のファイルに対応します。

```
① GET /documents/upload-url  → s3Key + 署名付き PUT URL (5分有効) を取得
② PUT <presignedUrl>         → テキストを S3 に直接アップロード（API Gateway を通過しない）
③ POST /documents { s3Key }  → SQS にキュー投入（メッセージは数百バイト）
   Worker Lambda             → S3 から読み込み → Bedrock Embeddings → S3 Vectors → S3 削除
```
